...
export default {
  ...
  methods: {
    validateRecentSearches() {
      ...
      let updatedArray = [];
      storedDataObject.forEach(async ({ result }, index) => {
        let timeDifference = this.timeDifferenceBetwenNowAndDate(result.date);
        if (timeDifference > 10) {
          let weatherData = await this.fetchWeatherDataForCity(result.name);
          const formattedData = this.parseApiResponse(newQuery, weatherData);
          this.updateLocalStorage(formattedData, index);
        }
      });
    },
    ...
    updateLocalStorage(updatedItem, index) {
